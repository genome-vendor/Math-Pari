...
in version 1.39.3.1. It should be noted that many of these functions accept
...
The expression C<x+y> is the I<sum> and C<x-y> is the I<difference> of
C<x> and C<y>.
...
The expression C<x*y> is the I<product> of C<x> and C<y>. Among the
...
row
vectors is not allowed, to obtain the I<scalar product> of two vectors of the
same
...
The expression C<x/y> is the I<quotient> of C<x> and C<y>. In addition to the
...
rational number (which may be an integer if the quotient is exact) and I<sl> the Euclidean quotient (see C<x\y> for that), and similarly the
...
to obtain the I<taylor series> expansion of the quotient of two polynomials,
...
The expression C<x\y> is the I<Euclidean quotient> of C<x> and C<y>.
...
The expression C<x%y> is the I<Euclidean remainder> of C<x> and C<y>. The 
...
The expression C<x^{ }y> is I<powering>. If the exponent is an integer,
then exact
...
=head2 comparison and I<boolean operators>.

The six standard I<comparison operators>
C<<=>, C<<>, C<E<gt>=>, C<E<gt>>, C<==>, C<!=> are available in GP, and in library mode 
under the names B<I<gle>, I<glt>, I<gge>, I<ggt>, I<geq>,
I<gne>> respectively. The 
...
The standard boolean functions C<| |> (I<inclusive or>) and C<&&> (I<and>) I<or>are
also available, and the library syntax is C<gor(x,y)> and
C<gand(x,y)> respectively. Note that to avoid confusion with the
factorial function, there is no C<!> (I<not>) operator, but this can easily be circumvented.
...
(C<x=1> ?), and I<gcmp_1> B<gcmp_1> C<(x)> (C<x=-1> ?).
...
and I<it> a GEN like C<gle> etc.



GP accepts the following synonyms for some of the above functions:
since there is no bitwise C<and> or bitwise C<or>, C<|> and C<&> are accepted asI<bitwise and> I<bitwise or>
synonyms of C<| |> and C<&&> respectively. Also, C<<E<gt>> is accepted as a
synonym for C<!=>. On the other hand, C<=> is definitely I<sl> a
...
I<sign> of C<x>, which must be of type
...
=head2 I<max>C<(x,y)> and B<I<min>>C<(x,y)>
...
assumes that C<x> and C<y> are PARI objects (the GEN type) and I<sl>
...
the files listed in C<I<gencom.h>>. Note that C<ldiv> is an
...
=head2 allocatememC<(x)>

this is a very special operation which
allows the user to change the stack size I<it> initialization.
...
Although it is a function, this must be the I<it> instruction in a GP
...
function C<taille> returns the number of I<it> instead.
...
=head2 I<components> of a PARI object
...
as follows: every PARI type has one or two initial I<code words>. The components
are counted,
...
(row or column) vector, then C<I<x[n]>> represents the C<n^{th}>
component of C<x>, i.e. C<compo(x,n)>. It is more natural and shorter to
write. If C<x> is a matrix, C<I<x[m,n]>> represents the coefficient of
row C<m> and column C<n> of the matrix, C<I<x[m,]>> represents
the C<m^{th}> I<sl> of C<x>, and C<I<x[,n]>> represents
the C<n^{th}> I<sl> of C<x>.
...
for polynomials and power series under GP.I<coeff (in library mode)>
...
C<gvar(x)> which returns the variable I<it> of C<x> if it 
...
Warning: this is I<sl> a substitution function. It is intended to be quick and
...
For real C<x>, C<n> is the number of desired significant I<sl> digits.
...
creates the quadratic number I<omega>
...
is the I<it> way to create reducible rationals (type 5) or rational
...
C<settyp> does I<it> create a copy of C<x>, while C<gsettype> does.
...
(or complex) number using the current I<precision> held in the variable
...
precision held in the variable B<I<precdl>>. Under GP this again is
...
square matrices, which are not implemented in the present version 1.39.3.1{}
...
well as to the more usual types.I<powering>
...
Contrary to most transcendental functions, an integer is I<it>
...
The library syntax is C<mpeuler(prec)> where C<prec> I<sl> be
given.
...
name B<I<geuler>> (with no parentheses) use instead C<consteuler(prec
)>.
...
1.39.3.1{} the C<p>-adic gamma function is not implemented.
...
C<I<incgam1> and I<incgam2>> which are used for internal purposes.
...
present version 1.39.3.1, this function is not very accurate when C<x> is
small.
...
C<x> are allowed in the present version 1.39.3.1.

The library syntax is C<kbessel(nu, x,prec)>.

Note: in addition, another implementation of this function which is often
faster than kbessel is the function B<I<kbessel2>>.



=head2 lnC<(x)> or B<log>C<(x)>

I<log> principal branch of the
...
In the present version 1.39.3.1{} the C<p>-adic lngamma function is not 
implemented.
...
The library syntax is C<mppi(prec)> where C<prec> I<sl> be given.
Note that this creates C<F<pi>> on the PARI stack. If one does not want to
create it on that stack but be able to use it later under the global
name B<I<gpi>> (with no parentheses) use instead C<constpi(prec)>.
...
In the present version 1.39.3.1{} a primitive but useful version of the I<ECM>
method has been implemented. Also, numbers found to be pseudo-primes after
10 successful trials of the I<Rabin-Miller> test are declared primes.
...
The present PARI version 1.39.3.1{} allows up to 20 user-specified
...
I<binomial coefficient> C<\binom x y>.
...
=head2 boundcfC<(x,lmax)>

creates the row vector whose components are
the partial quotients of the I<continued fraction> expansion of C<x>, the
...
In I<it> occasion can the program give a wrong answer: if the program
...
I<sl> is wanted. The numbers C<c> and C<c2> are positive real numbers
...
C<DE<gt>10^{10}> or when the I<sl> is wanted. If the integer C<n> is equal
...
(this case is not yet implemented in version 1.39.3.1).
...
creates the row vector whose components are
the partial quotients of the I<continued fraction> expansion of C<x>.
If C<x> is a real number, the expansion stops at the last significant partial
...
C<x>. In the present version 1.39.3.1, a simple algorithm is used for C<xE<gt>0>, so
...
There also exists the function C<I<classno2>>, which computes the class
number using the functional equation. However, it is in C<O(|x|^{1/2})>. 
Finally, in library mode only there exists the function C<I<classno3>> 
...
If you want only the I<sl> of the irreducible polynomials 
...
I<sl> of the irreducible polynomials (for example, for computing an
...
coefficients. In particular PARI does I<sl> yet know how to factor
multivariate polynomials.

The library syntax is C<factor(x)>. See also C<I<factpol>>,
C<I<factpol2>> and C<I<factornf>>.
...
I<Hurwitz class number> of C<x>, where C<x> is
...
=head2 hilbC<(x,y,p)> or B<hilbp>C<(x,y)>
...
I<fundamental unit> of the real quadratic field
...
global I<N\'eron-Tate height> of the
...
Note that in the present version 1.39.3.1, this function is accessible externally
...
version 1.39.3.1{}, this is implemented only for elliptic curves defined over
C<B<I<Q>>>.
...
C< F<exp> (2iF<pi> z)>. It is assumed that the curve is a I<it> Weil curve,
...
I<it> (usually C<x> or C<y>) will mean the following:

C<x> can be a C<B<I<Z>>>-basis, in Hermite Normal Form (HNF) or not. In this case
C<x> is a square matrix.

C<x> can be an I<it>, i.e. a 2-component vector, the first being an
...
is absolutely essential that all such vectors be I<it> vectors.
...
not the case, you I<it> absolutely change it into a C<B<I<Z>>>-generating
...
programs for working in I<it> extensions of number fields. To
...
A I<it> will be a matrix whose entries are elements
...
An I<it> will be a row vector of (fractional) ideals of the
number field C<nf>.

A I<it> will be a pair C<(A,I)> where C<A> is a relative
...
The I<it> generated by a pseudo-matrix C<(A,I)> is the sum
C<\sum_i\a_jA_j> where the C<\a_j> are the ideals of C<I> and C<A_j> is the
C<j>-th column of C<A>.

A pseudo-matrix C<(A,I)> is a I<it> of the module it
...
The I<it> of a pseudo-basis C<(A,I)> is the ideal
...
will I<it> launch C<buchinit> for you, which can be a costly
...
I<integral basis> of the number field defined by
...
discriminant of the number field (I<sl> of the polynomial C<x>).
...
given. However, they I<it> be given in the requested order. For example,
...
follows. We use the notations explained in the book by H. Cohen, I<it>, Graduate Texts in Maths
...
I<field discriminant> of the number field defined
...
=head2 factoredbasisC<(x,f)>

I<integral basis> of the number field 
...
of the polynomial C<x>. Note that C<f> does I<it> need to be a complete
...
=head2 factoreddiscfC<(x,p)>

I<field discriminant> of the number field defined by
...
as coefficients of C<x>, the variable of these polymods I<it> be the same
...
C<x\inB<I<Q>>[X]>. In the present version 1.39.3.1, C<x> must be irreducible and the
...
Note that this is I<it> the same as the LLL
...
=head2 isprincipalrayC<(bnfray,x)>
...
=head2 isprincipalraygenC<(bnfray,x)>
...
(which is well defined) is called the I<it> of the
module C<B<I<Z>>_L>.
...
convolution (or I<Hadamard product>) of the
...
The algorithm used is the I<subresultant algorithm>.
...
I<formal integration> of C<x> with respect to
...
=head2 isirreducibleC<(x)>

C<x> being a polynomial (univarariate
in the present version 1.39.3.1), returns 1 if C<x> is non-constant and 
...
creates the vector of the I<symmetric powers>
...
for C<2>-adic computations. Multiple roots are I<sl> repeated.
...
=head2 rootpadicC<(x,p,r)>

row vector of C<p>-adic roots of the polynomial
C<x> with C<p>-adic precision equal to C<r>. Multiple roots are I<sl> 
repeated.
...
I<adjoint matrix> of C<x>, i.e. the matrix C<y>
...
I<algebraic dependence> C<x> being real or complex, finds a
...
I<characteristic polynomial> of C<x> with respect
...
square matrix. Another program called B<I<det2>>C<(x)> is better
...
I<sl> of C<x>. The parameter C<y> must be either a number of type
...
maximal rank, finds the I<sl> Hermite Normal Form of C<x>
...
finds the I<sl> Hermite Normal Form of C<x>.
...
creates the I<Hilbert matrix> of order C<x>, i.e. the matrix whose
...
1.39.3.1, only independent rows are allowed. 
...
I<linear dependence>C<x> being a vector with real
...
are fast but as presently programmed (version 1.39.3.1) are numerically unstable.
...
numerical instability in the present version 1.39.3.1. An older version of
...
creates
as a matrix the lower triangular I<pascal triangle> of order C<x+1>
...
I<decomposition into squares> of the quadratic form
...
=head2 divsumC<(n,X,expr)>

sum of expression C<expr> over
the positive divisors of C<n>.

In the present version 1.39.3.1, C<n> is restricted to being less than C<2^{31}>.

The library syntax is I<divsomme>{(entree *ep, GEN num, char *ch)}.



=head2 hvectorC<(n,X,expr)>

creates a row vector with C<n> components
whose components are the expression C<expr> evaluated at the integral points
between 1 and C<n>.

The library syntax is I<vecteur>{(entree *ep, GEN nmax, char *ch)}.
...
Note that I<infinity> can be represented with essentially no loss of 
...
The library syntax is I<rombint>{(entree *ep, GEN a, GEN b, char *ch, long int prec)}.



=head2 intinfC<(X=a, b, expr)>

numerical integration from C<a> to C<b>,
tailored for being used when C<a> or C<b> are infinite. One I<sl> have 
C<abE<gt>0>, and in fact if for example C<b=\infty>, then it is preferable to have 
C<a> as large as possible, at least C<aE<gt>=1>.

The library syntax is I<qromi>{(entree *ep, GEN a, GEN b, char *ch, long int prec)}.



=head2 intnumC<(X=a,b,expr)>
...
The library syntax is I<qromb>{(entree *ep, GEN a, GEN b, char *ch, long int prec)}.



=head2 intopenC<(X=a,b,expr)>
...
The library syntax is I<qromo>{(entree *ep, GEN a, GEN b, char *ch, long int prec)}.
...
The library syntax is I<matrice>{(entree *ep1, entree *ep2, GEN nlig, GEN ncol, char *ch)}.



=head2 prodC<(x,X=a,b,expr)>
...
The library syntax is I<produit>{(entree *ep, GEN x, GEN a, GEN b, char *ch)}.



=head2 prodeulerC<(X=a,b,expr)>

product of expression C<expr>,
initialized at 1., the formal parameter C<X> ranging over the prime numbers
between C<a> and C<b>.I<Euler product>

The library syntax is I<prodeuler>{(entree *ep, GEN a, GEN b, char *ch, long int prec)}.



=head2 prodinfC<(X=a,expr)>

I<infinite product> of expression C<expr>, 
...
The library syntax is I<prodinf>{(entree *ep, GEN a, char *ch, long int prec)}.



=head2 prodinf1C<(X=a,expr)>
...
The library syntax is I<prodinf1>{(entree *ep, GEN a, char *ch, long int prec)}.



=head2 solveC<(X=a,b,expr)>

real root of expression C<expr> between C<a> and
C<b>, under the condition C<expr(X=a)*expr(X=b)E<lt>=0>. The method used is 
Brent's method.

The library syntax is I<zbrent>{(entree *ep, GEN a, GEN b, char *ch, long int prec)}.



=head2 sumC<(x,X=a,b,expr)>
...
The library syntax is I<somme>{(entree *ep, GEN x, GEN a, GEN b, char *ch)}.



=head2 sumaltC<(X=a, expr)>

numerical summation of the series C<expr>,
which should be an I<alternating series>, the formal variable C<X> 
...
The library syntax is I<sumalt>{(entree *ep, GEN a, char *ch, long int prec)}.



=head2 sumalt2C<(X=a, expr)>
...
The library syntax is I<sumalt2>{(entree *ep, GEN a, char *ch, long int prec)}.



=head2 suminfC<(X=a,expr)>

I<infinite sum> of expression C<expr>, the 
formal parameter C<X> starting at C<a>. The evaluation stops when the relative
error of the expression is less than the default precision. The expressions
must always evaluate to an element of C<B<I<C>>>.

The library syntax is I<suminf>{(entree *ep, GEN a, char *ch, long int prec)}.



=head2 sumposC<(X=a,expr)>
...
The library syntax is I<sumpos>{(entree *ep, GEN a, char *ch, long int prec)}.



=head2 vectorC<(n,X,expr)>
...
=head2 vvectorC<(n,X,expr)>

creates a column vector with C<n> components
whose components are the expression C<expr> evaluated at the integral points
between 1 and C<n>.

The library syntax is I<vvecteur>{(entree *ep, GEN nmax, char *ch)}.
...
The virtual cursor does I<it> move.
...
=head2 plotC<(X=a, b, expr)>

crude plot of the function represented by
expression C<expr> from C<a> to C<b>.

The library syntax is I<plot>{(entree *ep, GEN a, GEN b, char *ch)}.



=head2 plothC<(X=a,b,expr)>
...
The library syntax is I<ploth>{(entree *ep, GEN a, GEN b, char *ch, long int prec)}.



=head2 plothmultC<(X=a,b,[expr1,expr2,...])>
...
Note that in the present version 1.39.3.1{} this function has been written 
only for the X11-window system (hence also for GUI's based on X11 such as
Openwindows and Motif).

The library syntax is I<plothmult>{(entree *ep, GEN a, GEN b, char *ch, long int prec)}.



=head2 ploth2C<(T=a,b,expr)>
...
Note that in the present version 1.39.3.1{} this function has been written for
the Macintosh, the X11-window system (hence also for GUI's based on X11 such as
Openwindows and Motif), and for Sunview/Suntools. An Atari/gem version is also
available upon request.

The library syntax is I<ploth2>{(entree *ep, GEN a, GEN b, char *ch, long int prec)}.



=head2 plothrawC<(listx,listy)>
...
=head2 postplothC<(X=a,b,expr)>

same as C<ploth>, except that the 
output is a postscript program appended to the file C<pari.ps>.

The library syntax is I<postploth>{(entree *ep, GEN a, GEN b, char *ch)}.



=head2 postploth2C<(T=a,b,expr)>

same as C<ploth2>, except that the
output is a postscript program appended to the file C<pari.ps>.

The library syntax is I<postploth2>{(entree *ep, GEN a, GEN b, char *ch)}.



=head2 postplothrawC<(listx,listy)>
...
=head2 I<Variables> and symbolic expressions.
...
use non-alphanumeric symbols like '_', 'F<dollar>', or '.'. In addition to the
...
Now the main thing to understand is that PARI/GP is I<sl> a symbolic manipulation
...
when you use a variable name I<sl> assigning a value into it. This is perfectly
...
I<sl>, the simplest is just to write down the variables
...
something like C<x[k]=expr>). If C<x> is a matrix, you can assign a result to C<x[m,n]>, but I<sl> to C<x[m]>, which is a vector. If you want to assign an
...
=head2 I<Expression>s and I<expression sequence>s.
...
In fact, in the present version 1.39.3.1, the C<goto> instruction almost always
...
evaluated once (more generally, the condition on C<a> is tested I<sl> execution of
...
C<seq>. The test is made I<sl> evaluating the C<seq>, hence in particular if C<a>
...
C<pari.el>, when using GP under gnuemacs (see section 11) one I<sl>
...
Warning: in the present version 1.39.3.1, the printing is done on the standard
...
In a given session you can give any identifier name to a function, I<sl>
...
since version 1.34 the I<Riemann zeta-function> is included in the standard
...
=head2 Special I<editing character>s.
...
The first one is simply to use the I<backslash character> 'C<\>' at the end of the
...
It is the use of braces `C<{>' and  `C<}>'.I<brace characters>
...
=head2 The GP/PARI I<programming> language.
...
supplied, a special C<0> value called B<I<gnil>> is pushed on the stack
...
handled: a I<hashing function> is computed from the identifier, and used as an
index in C<I<hashtable>>, a table of pointers. Each of these pointers begins
a linked list of structures (type C<I<entree>>). The linked list is searched
...
=head1 Using GP under I<gnuemacs>.
...
what you have typed. Note that this will of course I<sl> work if for some
